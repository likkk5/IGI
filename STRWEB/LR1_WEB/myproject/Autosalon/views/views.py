...
import re
from ..models import AboutCompany, CartItem, Partner, Product
...
#                 messages.success(request, 'Промокод успешно применён!')
#             except Promo.DoesNotExist:
...
#     total_price = sum(item.total_price() for item in cart_items)  # Считаем общую стоимость
#     discount = Decimal('0')  # Начальное значение скидки
#     promo_code = None  # Для хранения применённого промокода

#     # Проверяем, был ли применён промокод
#     if request.method == 'POST':
#         promo_code = request.POST.get('promo_code', '').strip()  # Извлекаем промокод и убираем лишние пробелы
#         if promo_code:
#             try:
#                 # Проверяем существование активного промокода
#                 promo = Promo.objects.get(code=promo_code, is_active=True, expiration_date__gte=timezone.now())
#                 discount = promo.discount_value
#                 messages.success(request, 'Промокод успешно применён!')
#             except Promo.DoesNotExist:
...
#     discounted_price = total_price * (Decimal(1) - discount / 100)  # Убедитесь, что скидка в процентах

#     # Сохранение заказа в базе данных при подтверждении
...
#                 price=item.total_price() * (Decimal(1) - discount / 100)  # Учитываем скидку при формировании заказа
#             )

#         # Очищаем корзину после оформления заказа
#         cart.items.all().delete()  # Удаляем все товары из корзины
#         return redirect('customer_orders')  # Перенаправляем на страницу заказов
